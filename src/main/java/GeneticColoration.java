...
public class GeneticColoration{
     ...
     * Guarda la ruta ingresada en rutaArchivo, lee el archivo e inicializa la población
     */
    ...
    private void escribeSolucion(int[] solucion, int nPoblacion, double mutacion) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(nombreArchivoSalida))){
            ...
            writer.println("Solución: " + java.util.Arrays.toString(solucion) + " con " + evaluarSolucion(solucion) + " colores, " + nPoblacion + " individuos en la población y una tasa de mutación de " + mutacion);
    ...
    /**
     *  Método de selección de torneo para encontrar los individuos más aptos para reproducirse 
    ...
    private int[] torneo(List<int[]> poblacion) {
        ...
        for (int i = 0; i < tamanoTorneo; i++) {
            ...
            // Si la evaluación de la solución candidata es mejor que la mejor evaluación, actualizamos el mejor índice y evaluación
    ...
    /**
     * Verifica si una solución es válida, es decir, si dos vértices adyacentes no tienen el mismo color
    ...
    /**
     * Algoritmo genético elitista para encontrar la mejor solución al problema de coloración de grafos
    ...
    public int[] algoritmoGeneticoElitista(int numGeneraciones, double tasaMutacion) throws IOException {
        ...
        for (int generacion = 0; generacion < numGeneraciones; generacion++) {
            ...
            // Generamos una nueva población a partir hijos de las soluciones más aptas de la solución actual
            while (nuevaPoblacion.size() < poblacion.size() - 1) {
                ...
                for (int i = 0; i < hijos.length; i++) {
                    ...
                    if (esSolucionValida(hijos[i])) {
                        ...
                        if (nuevaPoblacion.size() == poblacion.size() - 1) break;
                    }
                }
            }
     ...
     */
    private void escribirDatosParaGnuplot(List<Integer> mejorEvaluacion, List<Double> promedioEvaluacion) throws IOException {
